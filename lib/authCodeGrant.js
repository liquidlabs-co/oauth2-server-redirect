...
function checkClient (done) {
  ...
  this.model.getClient(this.clientId, null, function (err, client) {
    ...
    if (!client) {
      ...
      if (client.redirectUri.indexOf(self.redirectUri.split("?")[0]) === -1) {
        return done(error('invalid_request', 'redirect_uri does not match'));
      }
      client.redirectUri = self.redirectUri;
    } else if (client.redirectUri !== self.redirectUri.split("?")[0]) {
      return done(error('invalid_request', 'redirect_uri does not match'));
    }
...
function redirect (done) {
  this.res.redirect(this.client.redirectUri + '?code=' + this.authCode +
      (this.req.query.state ? '&state=' + this.req.query.state : '')+(this.req.query.next_uri?"&redirect_uri="+encodeURIComponent(this.req.query.next_uri):""));
